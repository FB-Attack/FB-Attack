...
def find_most_popular_keyword(keywords, nweeks):
    ...
    for i in range(1, len(keywords), 4):
        ...
        print("({:d}/{:d}) Searching for:".format(i, len(keywords)),
              keywords_to_search, flush=True)
        pytrend.build_payload(kw_list=keywords_to_search)
        pytrend.build_payload(kw_list=keywords_to_search,
                              geo='US', timeframe='2020-01-01 2020-12-31')
        ...
        if idx_max < nread - 1:
            print("{:s} replaced {:s} as most popular!".format(
                keywords_to_search[idx_max], most_popular), flush=True)
    ...
    for i in range(0, nkw, 4):
        ...
        if baseline_keyword in keywords_to_search:
            ...
            pytrend.build_payload(
                kw_list=keywords_to_search + [baseline_keyword])
            data = pytrend.interest_over_time().to_numpy()
            mini_matrix = np.zeros((nread, nweeks))
            for i_kw in range(nread):
                for j, i_week in enumerate(range(-nweeks - 1, -1)):
        ...
        for k in range(0, nread):
            print(
                "{:s} ({:d}/{:d}) :".format(keywords[i + k], i + k, nkw), end='')
            print(trend_matrix[i + k][-5:], end='')
            print("    sum={:.2f}".format(
                sum(trend_matrix[i + k])), flush=True)
    return trend_matrix


def dataset_of_words_to_ids(dataset):
    unique_keywords = np.unique(
        [kw for document in dataset for kw in document])
...
def extract_words_from_original_dataset(dataset_original, dataset_date, dataset_length):
    ...
    for index, document in enumerate(dataset_original):
        ...
        unique_words_this_doc = list(
            set([word.lower() for word in unique_words_this_doc if word.isalpha()]))
...
def preprocess_raw_dataset(dataset_name, force_recompute=True):
    path_to_pre_dataset = os.path.join(
        PRE_DATASET_FOLDER, dataset_name + '.pkl')
    if os.path.exists(path_to_pre_dataset) and not force_recompute:
        print(
            "Pre-processed dataset {:s} already existed, not processing again.".format(dataset_name))
        return
    time0 = time.time()
    print("Going to process {:s}".format(dataset_name))
    if dataset_name == 'enron-full':
        path_to_raw_dataset = os.path.join(
            RAW_DATASET_FOLDER, 'enron_mail_20150507.tar.gz')
        dataset_original = []
        dataset_date = []
        dataset_length = []
        count = 0
            ...
            for member in tar.getmembers():
                if '_sent_mail' in member.path:
                    if member.isfile():
                        ...
                        email_context, email_date = process_email_enron(
                            message)
        ...
        print("Done reading, we have {:d} documents".format(
        ...
        results = (dataset, unique_keywords,
                   dataset_newdate, dataset_newlength)
        ...
        for filename in os.listdir(path_to_raw_dataset):
            if filename.endswith('mbox'):
                email_date = datetime.datetime(
                    year=int(filename[0:4]), month=int(filename[4:6]), day=1)
                mbox = mailbox.mbox(os.path.join(
                    path_to_raw_dataset, filename))
                for message in mbox:
                    dataset_original.append(process_email(
                        message).split("To unsubscribe")[0])
                    dataset_length.append(
                        len(process_email(message).split("To unsubscribe")[0].encode()))
                    dataset_date.append(email_date)
                    count += 1
                    if count % 1000 == 0:
                        print("{:d} documents processed".format(count))
...
def process_pre_dataset(dataset_name, nkw=3000, force_recompute=True):
    global chosen_stemid, old_to_new_stemid
    path_to_pro_dataset = os.path.join(
        PRO_DATASET_FOLDER, dataset_name + '.pkl')
    if os.path.exists(path_to_pro_dataset) and not force_recompute:
        print("Processed dataset {:s} already existed, not processing again.".format(
            dataset_name))
        return
    time0 = time.time()
    print("Going to process {:s}".format(dataset_name))
    ...
    kwid_to_stemid = {i: stem_vocabulary_unique.index(stem)
    ...
    new_dataset = [sorted(list(set([kwid_to_stemid[kwid]
                                    for kwid in document if kwid in kwid_to_stemid])))
    ...
    if dataset_name == "enron-full":
        chosen_stemid, old_to_new_stemid = select_top_keyword(
            dataset_name, 2001, 6, 3000)
    if dataset_name == "lucene":
        chosen_stemid, old_to_new_stemid = select_top_keyword(
            dataset_name, 2007, 12, 3000)
    chosen_stems = [stem_vocabulary_unique[chosen_stemid[i]]
                    for i in range(nkw)]
    ...
    for word, stem in zip(vocabulary, stem_vocabulary):
        if stem in stems_to_words:
            ...
            results = (new_dataset, chosen_stems, {
                       'stems_to_words': stems_to_words})
    results = (chosen_stemid, old_to_new_stemid, chosen_stems,
               {'stems_to_words': stems_to_words})
    with open('../datasets_mon/'+str(dataset_name)+'chonse_keyword.pkl', 'wb') as f:
        pickle.dump(results, f)
    new_dataset = [sorted(list(set([old_to_new_stemid[stemid] for stemid in document
                                    if stemid in old_to_new_stemid]))) for document in new_dataset]
    print("Done choosing top stem ID's and trimming dataset, current size {:d}".format(
        len([id for document in new_dataset for id in document])))
    print(
        "Dataset after removing empty documents: {:d} -> ".format(len(new_dataset)), end="")
    new_dataset_date = [date for index, date in enumerate(
        dataset_date) if len(new_dataset[index]) > 0]
    new_dataset_length = [length for index, length in enumerate(
        dataset_length) if len(new_dataset[index]) > 0]
    new_dataset = [document for document in new_dataset if len(document) > 0]
    print("{:d} documents".format(len(new_dataset)))
    print("{:d} dates".format(len(new_dataset_date)))
    print("{:d} length".format(len(new_dataset_length)))
    keyword_dict = {}
    for index, document in enumerate(new_dataset):
        for word in document:
            if word in keyword_dict:
                keyword_dict[word].append(new_dataset_length[index])
            else:
                keyword_dict[word] = []
                keyword_dict[word].append(new_dataset_length[index])
...
def select_top_keyword(dataset_name, year, month, nkw):
    new_dataset, dataset_date, dataset_length = load_data_bymonth(
        dataset_name, year, month)
    stemid_counter = Counter(
        [stemid for stemid_list in new_dataset for stemid in stemid_list])
    chosen_stemid = [stemid for stemid, n in stemid_counter.most_common(nkw)]
    old_to_new_stemid = {old: new for new, old in enumerate(chosen_stemid)}
    return chosen_stemid, old_to_new_stemid


def pro_dataset_top_id(old_to_new_stemid, new_dataset, dataset_date, dataset_length):
    new_dataset = [sorted(list(set([old_to_new_stemid[stemid] for stemid in document
                                    if stemid in old_to_new_stemid]))) for document in new_dataset]
    print("Done choosing top stem ID's and trimming dataset, current size {:d}".format(
        len([id for document in new_dataset for id in document])))
...
def save_enron_data_bymonth(dataset_name, dataset, dataset_date, dataset_length):
    save_path = '../datasets_mon/' + dataset_name + '/'
    ...
    for index, data in enumerate(dataset_date):
        year = data.year
        month = data.month
        locals()[str(year) + "_" + str(month) +
                 "dataset"].append(dataset[index])
        locals()[str(year) + "_" + str(month) +
                 "date"].append(dataset_date[index])
        locals()[str(year) + "_" + str(month) +
                 "length"].append(dataset_length[index])
    res_dataset = []
    res_dataset_date = []
    res_dataset_length = []
    for i in year_list:
        for j in month_list:
            ...
            res_dataset_date.extend(
                locals()[str(i) + "_" + str(j) + "dataset"])
            res_dataset_length.extend(
                locals()[str(i) + "_" + str(j) + "length"])
...
def load_data_bymonth(dataset_name, year, month):
    save_path = '../datasets/' + dataset_name + \
        '/' + str(year) + "_" + str(month) + ".pkl"
...
def get_frequencies_from_google_trends(dataset_list):
    ...
    def load_keyword_trends():
        path_to_keyword_trends = os.path.join(
            PRO_DATASET_FOLDER, 'trends0_1500.pkl')
        if os.path.exists(path_to_keyword_trends):
            with open(path_to_keyword_trends, "rb") as f:
                keyword_trends = pickle.load(f)
            print("Loaded keyword_trends, we have {:d} keywords".format(
                len(keyword_trends), flush=True))
        else:
            keyword_trends = {}
    ...
    for dataset_name in dataset_list:
        ...
        keywords = [kw for kw_list in stems_to_words.values()
                    for kw in kw_list]
        for kw in keywords:
            if kw not in keyword_trends and kw not in new_keywords:
                new_keywords.append(kw)
    print("Done scanning the datasets, we have {:d} new keywords".format(
        len(new_keywords)), flush=True)
    if len(new_keywords) > 0:
        ...
        for i, kw in enumerate(new_keywords):
            ...
            print("Added '{:s}', {:d} left".format(
                kw, len(new_keywords) - i - 1), flush=True)
        save_keyword_trends(keyword_trends)
...
def add_frequency_trends_information_to_dataset(dataset_name):
    ...
    if not all([word in keyword_trends for word in words]):
        print("Not all words in {:s} are in the trend dictionary, we skip this dataset".format(
            dataset_name))
    elif 'trends' in aux:
        ...
        for i_stem, stem in enumerate(stems):
            for word in aux['stems_to_words'][stem]:
                if isinstance(keyword_trends[word], pd.DataFrame):
                    trends_matrix[i_stem,
                                  :] += keyword_trends[word]['max_ratio'].values
