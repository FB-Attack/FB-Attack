...
def run_single_experiment(exp_params):
    def _generate_train_test_data(dataset_name, n_keywords):
        chosen_keywords, trend_matrix = load_stem_trends(
            dataset_name, n_keywords)

        trend_matrix_norm = trend_matrix.copy()
        for i_col in range(trend_matrix_norm.shape[1]):
            if sum(trend_matrix_norm[:, i_col]) == 0:
                print(
                    "The {d}th column of the trend matrix adds up to zero, making it uniform!")
                trend_matrix_norm[:, i_col] = 1 / n_keywords
            else:
                trend_matrix_norm[:, i_col] = trend_matrix_norm[:,
                                                                i_col] / sum(trend_matrix_norm[:, i_col])
        return trend_matrix_norm, chosen_keywords

    trend_real_norm, chosen_keywords = _generate_train_test_data(
        exp_params['dataset'], exp_params['nkw'])
...
def static_enron_data_info(dataset_name, nkw, trend_norm, n_month, query_number, delete_rate):
    ...
    for i in range(n_month):
        ...
        temp_del_size = delete_get_size_frompkl(
            dataset_name, year, month, nkw, delete_rate)
    ...
    for month_count in range(n_month):
        ...
        if month_count == 0:
            ...
            query_keyword = np.floor(
                query_number * trend_norm[:, month_count]).astype(int)
            # print(query_keyword)
            query_keyword_list = []
            for i in range(nkw):
                query_keyword_list.extend(
                    [i for npt in range(query_keyword[i])])
            ...
            for q_id in query_keyword_list:
                ...
                if in_flag == 0:
                    temp_class[q_id] = []
                    temp_class[q_id].append(
                        all_keyword_size[month_count][id_to_word[q_id]])
                    temp_class_count[q_id] = 1

            # print(temp_cand)
            for k2, v2 in temp_class.items():
                cand_np = np.abs(
                    temp_class_count[k2] - statistic_trend_number[:, month_count].astype(int))
                ...
                for i_m in range(month_count):
                    if keyword in delete_keyword_size[i_m].keys():
                        keyword_size[keyword].append(
                            delete_keyword_size[i_m][keyword])
                    else:
                        keyword_size[keyword].append([])
    ...
    for i in range(1, n_month):
        for k1, v1 in counter_size[i].items():
            ...
            if in_flag == 1:
                new_candidate[max_key] = candidate[i][k1].intersection(
            ...
            # if in_flag == 0:
                ...
                if k1 not in new_candidate_size.keys():
                    ...
                    new_candidate_count[k1 +
                                        3000] = candidate_class_count[i][k1]
                    ttc[k1 + 3000] = []
        ...
        for query_number in query_list:
            for n_month in query_month:
                ...
                ans = static_enron_data_info(
                    'lucene', nkw, trend_norm, n_month, query_number, delete_rate=0.00)
